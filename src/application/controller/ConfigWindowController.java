...
public class ConfigWindowController implements Initializable {
    ...
    void handleSearchSourceLocation(ActionEvent event) {
        String title = "Bitte wählen Sie das Verzeichnis aus,"
                + " in dem die zu archivierenden Dateien liegen";
    ...
    void handleTakeChange(ActionEvent event) {
        ...
        if (source.exists() && destination.exists()) {
            con.insertConfig(labelPathSourceLocation.getText(),
                    labelPathDestinationLocation.getText(), "true"); // Version
    ...
    private void useDirChooser(Label lbl, String title) {
        /*
         * kerstin: damit nicht mehrmals faktisch derselbe Code dasteht eigene Methode es wird ein
         * Directory ausgewählt und das Ergebnis in das entsprechende Label geschrieben. Bricht der
         * Anwender die Auswahl ab, passiert nix
         */
