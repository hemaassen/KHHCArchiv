...
public class ManualWindowController implements Initializable {
	...
	public void handleSearchDoc() {
		...
		if (sourceFileName != null && !sourceFileName.canWrite()) {
			...
			dialog.setContentText("Bitte wählen Sie nur Dokumente aus für die Sie auch Schreibrechte haben! "
		...
		if (sourceFileName != null) {
			...
			try {
				if (isPdf) {
					...
					Image myImage = new Image(sourceFileName.toURI().toURL().toExternalForm(), 595.0, 842.0, false,
							true);
					zoomProperty.set(200);
					imageActualDoc.setImage(
	...
	private void setPathToDestination(int level) {
		...
		try{
			...
			case 5:
				...
				break;
			default:
				throw new IllegalArgumentException("setPathToDestination mit falschen Paramter aufgerufen");
			}
		...
		manualLabelDestinationPath
				.setText(main.getMyConfig().getDestinationDir().toString() + File.separator + pathToDestination);
	}
