...
public class SearchWindowController implements Initializable {
    ...
    void searchContent(ActionEvent event) {
            ...
            try {
                if (dateFrom.getValue().compareTo(dateTill.getValue()) > 0) {
                    ...
                    alert.setContentText(
                            "Datum bis darf nicht jünger als Datum von sein");
                    alert.showAndWait();
                }
            } catch (Exception e) {
        ...
        // Pfad für die Suche aus den ausgewählten Schlüsselwörtern
        // zusammensetzen
        ...
        try {
            ...
            listResult.setOnMouseClicked(new EventHandler<MouseEvent>() {
                ...
                public void handle(MouseEvent event) {
                    ...
                    myChoosenFile = new File(
                            myPath + File.separator + choosenFileString);
                    // ausgewählte Datei anzeigen mit Zoommöglichkeit über
                    // Mausrad
                    try {
                        if (isPdf) {
                            fxImage = PdfHelper
                                    .convertPDFToImage(myChoosenFile);
                            zoomProperty.set(200);
                            choosenDoc.setImage(ZoomHelper.zoomMouse(fxImage,
                                    choosenDoc, anchorSearchMain,
                                    imageScrollPane, zoomProperty));
                        } else {
                            Image myImage = new Image(
                                    myChoosenFile.toURI().toURL()
                                            .toExternalForm(),
